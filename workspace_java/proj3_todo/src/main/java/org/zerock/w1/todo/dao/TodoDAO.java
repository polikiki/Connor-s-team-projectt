...
public class TodoDAO {
	...
	private Connection getConnection() {
		...
		try {
			...
			// 드라이버 : 서로 다른 것들(java,oracle)이 소통할 수 있게 해주는것
			// 오라클 드라이버 로딩
	...
	public List<TodoDTO> selectAll() {
		...
		try {
			...
			// executeQuery : select 전용, Result을 반환함
			// executeUpdate : selsect 외 모든것, int를 반환함
...
//    		java.sql.Date
	...
	public int insert(TodoDTO dto) {
		...
		try {
			// ctrl + shift + o로 import할꺼를 편하게 고를 수 있다
			// Servers 폴더의 context.xml에서
			// name이 jbc/oracle인 resource를 가져와서 DataSource로 저장하기
