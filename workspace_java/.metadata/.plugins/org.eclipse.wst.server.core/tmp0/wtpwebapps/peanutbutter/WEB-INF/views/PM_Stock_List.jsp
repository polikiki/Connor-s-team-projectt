...
<body>
	<div class="container">
		<div class="top">
			<img src="image/directory.png" style="width: 30px;"> <span>완제품 재고 관리</span> 
		<form id="searchform" action="pm_stock" method="get">	
		...
		<div class="section">
			<table class="table">
				...
				<tbody class="tbody">
					<c:forEach var="PM_Stock" items="${map.list }">
						<tr>
							<td><input type="checkbox" value="${PM_Stock.product_code}"></td>
							<td>${PM_Stock.product_code }</td>
							<td>${PM_Stock.product_name }</td>
							<td><img src="${PM_Stock.product_image}"></td>
							<td>${PM_Stock.stock_amount}</td>
							<td>${PM_Stock.shipping_warehouse}</td>						
							<td>${PM_Stock.latest_stock_date}</td>
							<td><img src="${PM_Stock.barcode}"></td>
							<td><select id = select_status name="optionvalue_status" onchange="take(this, '${PM_Stock.product_code}')">
                                <option value="-----"
                                	<c:if test="${empty map.optionvalue || map.optionvalue == '-----'}">selected</c:if>>-------</option>
                                <option value="stay"
                                	<c:if test="${map.optionvalue == 'stay'}">selected</c:if>>출고대기</option>
                                <option value ="underway"
                                	<c:if test="${map.optionvalue == 'underway'}">selected</c:if>>출고 진행중</option>
                                <option value ="complete"
                                	<c:if test="${map.optionvalue == 'complete'}">selected</c:if>>출고 완료</option>
                            </select>
                            </td>
						</tr>
					</c:forEach>
	...
	<form id="deleteone" action="delete_pm_stock" method="post">
...
<script>
    ...
    function searchpage(num){
    	...
    	let url = "http://127.0.0.1:8080/peanutbutter/pm_stock?page=" + num;
    	
    	
    	if(a != ''){    		
//     		location.href="http://127.0.0.1:8080/peanutbutter/pm_stock?optionvalue="+a+"&search="+b+"&page=num";
