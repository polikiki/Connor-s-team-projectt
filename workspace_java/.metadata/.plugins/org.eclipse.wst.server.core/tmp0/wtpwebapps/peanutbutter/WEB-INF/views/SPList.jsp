...
<body>
    <div class="container">
        ...
        <div class="search_container">
            <div class="search">
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
             <form id="searchform" action="sp" method="get">
                <select id="select" name="optionvalue" class="inputTextHeight">
                    <option value="-----"
						...
						<option value="sp_number"
							<c:if test="${map.optionvalue == 'sp_number'}">selected</c:if>>작업지시서 NO</option>
            ...
            <table class="table">
                ...
                <tbody class="tbody">
                	<c:forEach var="SP" items="${map.list }">
                    	<tr>
                        	<td><input type="checkbox" class="delete-checkbox" value="${SP.specification_number}"></td>
                        	<td>${SP.specification_number}</td>
                        	<td><a href="orderDetail.html">${SP.product_name}</a></td>
                        	<td><input type="button" value="확인" onclick="BOM()"></td>
                        	<td>${SP.plan_start_date}</td>
                        	<td>${SP.plan_reg_date}</td>
                        	<td>${SP.plan_amount}</td>
                        	<td>
                            <select id = select_status name="optionvalue_status" onchange="take(this, '${SP.specification_number}')">
                                <option value="-----"
                                	<c:if test="${empty map.optionvalue || map.optionvalue == '-----'}">selected</c:if>>-------</option>
                                <option value="stay"
...
function searchTable() { // 돋보기 버튼을 클릭했을 때 동작하는 함수
    ...
    console.log('Specification Numbers:', codesField.value);
    ...
    function searchpage(num){
        ...
        let url = "http://127.0.0.1:8080/peanutbutter/sp?page=" + num;
        	
        	
        if(a != ''){    		
//         	location.href="http://127.0.0.1:8080/peanutbutter/sp?optionvalue="+a+"&search="+b+"&page=num";
    ...
    function take(selectElement, specificationNumber) {
    ...
    const selectedValue = selectElement.value;
        ...
        httpRequest.open("put", "/peanutbutter/update_sp?specification_status="+selectedValue+"&specification_number="+specificationNumber, true);
