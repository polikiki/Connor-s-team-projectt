...
<body>
    <div class="container">
        <div class="top">
            <img src="image/directory.png" style="width: 30px;"> <span>부품재고 관리</span>
        <form id="searchform" action="pt_stock" method="get"> 
            <select class="select" name=optionvalue>
                <option value="-----" <c:if test="${empty map.optionvalue || map.optionvalue == '-----'}">selected</c:if>>-----</option>
                <option value="part_name" <c:if test="${map.optionvalue == 'partname'}">selected</c:if>>부품코드</option>
                <option value="warehouse" <c:if test="${map.optionvalue == 'warehouse'}">selected</c:if>>부품분류</option>
                <option value="stock_date" <c:if test="${map.optionvalue == 'stock_date'}">selected</c:if>>모델명</option>
            </select>
        ...
        <div class="section">
            <table class="table">
                ...
                <tbody class="tbody">
                <c:forEach var="PT_Stock" items="${map.list }">
                    <tr>
                        <td><input type="checkbox" class="delete-checkbox" value="${PT_Stock.part_code}"></td>
                        <td>${PT_Stock.part_code}</td>
                        <td>${PT_Stock.part_name}</td>
                        <td>${PT_Stock.part_modelname}</td>
                        <td><img src="${PT_Stock.part_image}"></td>
                        <td>${PT_Stock.stock_amount}</td>
                        <td>${PT_Stock.receving_warehouse}</td>
                        <td>${PT_Stock.latest_stock_date}</td>
                        <td>${PT_Stock.part_vendor1},${PT_Stock.part_vendor2}</td>
                        <td><img src="${PT_Stock.part_barcode}"></td>
                    </tr>
                </c:forEach>
             </tbody>
           </table>
	...
	<form id="deleteone" action="delete_pt_stock" method="post">
...
<script>
    ...
    function searchpage(num){
    	...
    	let url = "http://127.0.0.1:8080/peanutbutter/pt_stock?page=" + num;
