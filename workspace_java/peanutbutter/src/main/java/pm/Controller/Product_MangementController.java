...
import pm.service.Product_Management_Page_Service;


@WebServlet("/pm")
public class Product_MangementController extends HttpServlet {
	private static final long serialVersionUID = 1L;

	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		
		System.out.println("/pm doget 실행");
...
//		List list = pmService.getPM(Product_Code);
		...
		Product_Management_Page_Service pmPageService = new Product_Management_Page_Service();
		Map map = pmPageService.getpmPage(countPerPage, page, search, optionvalue);
...
//		request.setAttribute("list", list); // pageService 및 DAO를 만들면서 PMList에서는 쓸일이 없어 졌음
		...
		request.getRequestDispatcher("/WEB-INF/views/PMList.jsp").forward(request, response);
