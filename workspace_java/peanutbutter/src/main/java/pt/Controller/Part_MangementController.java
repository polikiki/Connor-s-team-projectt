...
import pt.service.Part_Management_Page_Service;


@WebServlet("/pt")
public class Part_MangementController extends HttpServlet {
	private static final long serialVersionUID = 1L;

	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		
		System.out.println("/pt doget 실행");
...
//		List list = pmService.getPM(Product_Code);
		...
		Part_Management_Page_Service ptPageService = new Part_Management_Page_Service();
		Map map = ptPageService.getptPage(countPerPage, page, search, optionvalue);
...
//		request.setAttribute("list", list); // pageService 및 DAO를 만들면서 PMList에서는 쓸일이 없어 졌음
		...
		request.getRequestDispatcher("/WEB-INF/views/PTList.jsp").forward(request, response);
